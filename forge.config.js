...
function fileExists(path) {
    return new Promise((resolve, reject) => {
        fs.access(path, constants.F_OK)
            ...
            .catch((err) => {
                console.log(`${path} Doesn't Exist. Start download.`)
                resolve(false)
            })
    })
}
    ...
    hooks: {
        generateAssets: async (forgeConfig, platform, arch) => {
            ...
            try {
                ...
                const frankenphp = path.join(process.cwd(), 'bin', 'frankenphp')
                const frankenExists = await fileExists(frankenphp);
                if (!frankenExists) {
                    execSync('curl https://frankenphp.dev/install.sh | sh', {
                        cwd: path.join(process.cwd(), 'bin'),
                        stdio: 'inherit'
                    })
                }

                const phpBackend = path.join(process.cwd(), 'bin', 'php_backend')
                const backendExists = await fileExists(phpBackend);
